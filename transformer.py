...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_position_embeddings):
        super().__init__()
        self.encoding = torch.zeros(max_position_embeddings, d_model)
        self.encoding.requires_grad = False

        pos = torch.arange(0, max_position_embeddings)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, hidden_states, dec_self_mask = None, enc_hidden_states = None, enc_dec_mask = None, past_key_value = None):
        '''
            Inputs
        ...
        residual = hidden_states
        hidden_states, cross_attn_present_key_value = self.cross_attn(
            query_states = hidden_states,
        ...
        '''
            Inputs
                ...
                Case 2 : Inference
                    ...
                    enc_hidden_states.shape = (B, E_L, H)
                    enc_mask.shape = (B, E_L)
                    past_key_values = (self_attn_key_value, cross_attn_key_value)
...
class TransformerForConditionalGeneration(nn.Module):
    def __init__(self, cfg, tokenizer):
        ...
        self.lm_head = nn.Linear(cfg.d_model, cfg.plm_vocab_size, bias = False)

    def forward(self, enc_ids, enc_mask, dec_ids, dec_mask = None, enc_hidden_states = None, past_key_values = None, label_ids = None):
